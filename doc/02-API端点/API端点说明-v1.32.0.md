...
| v1.32.0 | 2026-09-24 | **积分底座上线**（对齐《11-积分体系设计 v1.0.0》）：新增积分组 3 端点 `GET /api/points/account`（余额/累计/今日签到态/连击数）、`POST /api/points/checkin`（阶梯签到）、`GET /api/points/transactions`（流水分页）；Admin 组新增 2 端点 `GET/PUT /api/admin/points/rules[/{id}]`（赚分规则配置，system.manage 权限，实时生效）；端点总数 151→156。赚端 5 场景（登录/签到/注册/纠错采纳/入驻通过）经 PointsService 唯一写入口接线，扣分入口就位场景留白。 |
...
OpenFindBearings.Api（以下简称 API）共注册 **148** 个端点，按职责划分为 8 组。另提供内部配置端点 `/api/config/reliability` 供 Sync 拉取可信度阈值（不计入 8 组统计）。
