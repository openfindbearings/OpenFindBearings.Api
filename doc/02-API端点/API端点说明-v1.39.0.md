...
## 变更记录
| v1.39.0 | 2026-09-26 | 权限目录重排端点侧改键（端点数量不变 167）：品牌端点组 bearing.*→brand.view/create/edit/delete、类型端点组→type.*、points rules→points.manage、按-auth 角色读/assign/移除与 provision→user.assign（Admin 专属防提权）、批量角色映射→user.view；Admin 端 reset-to-default 新端点属 Identity API 不计入本清单；键目录全表见 04-用户角色权限设计 v1.7.0 |
