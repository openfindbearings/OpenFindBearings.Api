...
OpenFindBearings.Api（以下简称 API）共注册 **110 ** 个端点，按职责划分为 8 组。
...
| 后台管理 | `/api/admin` | 46 | Bearer（Admin 策略）+ RequirePermission |
| SysAdmin | `/api/sysadmin` | 1 | Bearer（Admin 策略）+ RequireRole |
| API 配置文件 | `/api/config` | 5 | Bearer（Admin 策略）+ RequirePermission |
| 同步 | `/api/sync` | 6**+1** | Bearer（SyncClient 策略） |

> **版本说明：** `/api/merchants/batch-check-verified` 路由在 `/api` 前缀但认证策略为 SyncClient（仅限 L 阶段调用），不对外公开。
...
| GET | `/api/me/favorites/merchants` |
| POST | `/api/me/favorites/merchants/{id}` |
| DELETE | `/api/me/favorites/merchants/{id}` |
| GET | `/api/me/favorites/merchants/{id}/check` |
...
| PUT | `/api/admin/users/{id:guid}` | `user.manage` |
| DELETE | `/api/admin/users/{id:guid}` | `user.manage` |
| PUT | `/api/admin/users/{id:guid}/restore` | `data.restore` |
| GET | `/api/admin/users/me/profile` | `user.manage` |
| PUT | `/api/admin/users/me/profile` | `user.manage` |
| GET | `/api/admin/users/me/roles` | `user.manage` |
| GET | `/api/admin/users/me/permissions` | `user.manage` |

### 6.9 ETL 触发

| 方法 | 路由 | RequirePermission |
|------|------|-------------------|
| POST | `/api/admin/etl/trigger` | `sync.trigger` |
| POST | `/api/admin/etl/trigger/{phase}` | `sync.trigger` |

### 6.10 爬虫管理

| 方法 | 路由 | RequirePermission |
|------|------|-------------------|
| GET | `/api/admin/crawlers` | `crawler.view` |
| GET | `/api/admin/crawlers/{name}/status` | `crawler.view` |
| POST | `/api/admin/crawlers/{name}/run` | `crawler.trigger` |
...
| POST | `/api/admin/roles/{roleName}/permissions` | `role.manage` |
...
| GET | `/api/admin/config/{key}` | `system.view` |
| PUT | `/api/admin/config/{key}` | `system.manage` |

---

## 7. 同步端点 `/api/sync`（6 + 1 个）
...
| POST | `/api/merchants/batch-check-verified` | 批量查询商户是否已认证 | `BatchCheckMerchantsVerified` |

> `POST /api/merchants/batch-check-verified` 路由注册在 `/api` 前缀下，但认证策略使用 SyncClient，仅在 L 阶段内部调用。
