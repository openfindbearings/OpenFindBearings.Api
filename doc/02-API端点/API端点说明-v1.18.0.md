...
> - 五键由 SeedData 播种，存量库经 `EnsureConfigKeysAsync` 启动时幂等补全（无 EF 迁移）。发布新版流程（v1.18.0 起为集群拉取式）：Publish Release（规范 tag `vX.Y.Z[-rc.N]`）→ Taro CI 构建 APK 传 Release（跨境推包/写库步骤已删除）→ K3s CronJob `openfindbearings-apk-sync` 每晚 1 点（北京时间）拉最新规范版本分包（sha256 校验、断点续传），两包齐备才改写 `Mobile.AppVersion`、写固定模板 `Mobile.UpdateMessage`；拉不完不宣告、下轮续传。Admin 系统配置页仅作手动兜底。
