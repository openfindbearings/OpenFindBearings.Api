    ...
    public class GetMobileHomeQueryHandler : IRequestHandler<GetMobileHomeQuery, MobileHomeDto>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var result = new MobileHomeDto
            {
                ...
                RecommendedMerchants = merchantList.Select(m => new MerchantLightDto
                {
                    Id = m.Id,
                    Name = m.Name,
            ...
            {
                result.UserStats = new MobileUserStatsLightDto
                {
        ...
        private async Task<List<MobileBannerLightDto>> GetBannersAsync(
            List<Domain.Entities.SystemConfig> configs,
            CancellationToken cancellationToken)
        {
            var banners = new List<MobileBannerLightDto>();
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                banners.Add(new MobileBannerLightDto
                {
                    Id = i.ToString(),
                    Title = title,
                    ImageUrl = imageUrl,
        ...
        private async Task<List<MobileCategoryLightDto>> GetCategoriesAsync(
            List<Domain.Entities.SystemConfig> configs,
            CancellationToken cancellationToken)
        {
            var categories = new List<MobileCategoryLightDto>();
            ...
            foreach (var id in ids)
            {
                ...
                categories.Add(new MobileCategoryLightDto
                {
                    Id = id,
                    Name = name,
                    IconUrl = iconUrl
