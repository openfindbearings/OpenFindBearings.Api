...
namespace OpenFindBearings.Application.Features.Favorites.EventHandlers
{
    ...
    /// 轴承被收藏事件处理器
    /// </summary>
    public class BearingFavoritedEventHandler : INotificationHandler<BearingFavoritedEvent>
    {
        private readonly ILogger<BearingFavoritedEventHandler> _logger;
        private readonly ICacheService _cacheService;

        public BearingFavoritedEventHandler(
            ILogger<BearingFavoritedEventHandler> logger,
        ...
        public async Task Handle(BearingFavoritedEvent notification, CancellationToken cancellationToken)
        {
            _logger.LogInformation(
                "轴承被收藏: 用户ID={UserId}, 轴承ID={BearingId}, 时间={OccurredOn}",
                notification.UserId,
